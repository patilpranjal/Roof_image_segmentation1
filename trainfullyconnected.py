...
# Load datasets.
import pandas as pd
df=pd.read_csv(trainDataFileName, sep=',',header=None)
df1=pd.read_csv(testDataFileName, sep=',',header=None)
print (df.shape)

X_train = (df.values[:,0:75]).transpose()
Y_train = df.values[:,-1].reshape((1,X_train.shape[1]))
X_test = df1.values[:,0:75].transpose()
Y_test = df1.values[:,-1].reshape((1,X_test.shape[1]))
...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1", [100,75], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.0001,
          num_epochs = 200, minibatch_size = 2, print_cost = True):
    """
